...
public class JobScenarioFactory {
    ...
    //Min. and max. events expected for each batch job.
    ...
    //Min. and max. duration expected for each batch job.
    ...
    public List<BatchEvent> buildNormal(JobType jobType) {
        ...
        double duration = randomDouble(DURATION_RANGES.get(jobType));
        String source = jobType.name().toLowerCase() + "_export.csv";

        return buildEvents(runId, jobType, rows, duration, 0, source);
    }

    public List<BatchEvent> buildAnomalous(JobType jobType) {
        String runId  = "run-anomaly-" + System.currentTimeMillis();
        long   rows   = randomLong(ROW_RANGES.get(jobType));
        double duration  = randomDouble(DURATION_RANGES.get(jobType)) * AnomalyInjector.DURATION_MULTIPLIER;
        int    errors = (int) (rows * AnomalyInjector.ERROR_RATE);
        String source = jobType.name().toLowerCase() + "_export.csv";

        return buildEvents(runId, jobType, rows, duration, errors, source);
    }
    ...
    private List<BatchEvent> buildEvents(String runId, JobType jobType,
        ...
        events.add(event(jobType, new JobProgress(runId, rows / 3, 33))); //33% done
        events.add(event(jobType, new JobProgress(runId, rows * 2 / 3, 66))); //66% done
        events.add(event(jobType, new JobCompleted(runId, rows, durationSeconds, errorCount, source))); // finished
